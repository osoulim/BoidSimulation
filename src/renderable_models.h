...
//
	...
	template <typename View>
	void render(ParticleBarrierModel const &model, View const &view) {
		// static: only initiallized once, the first time this function is called.
		// This isn't the most elegant method to do this but it works. Just put your
		// draw calls in here.
		...
		static auto barrierRenderable = createInstancedRenderable(
				Sphere(Radius(1.0)),
				NoShading(Colour(0.1f, 0.1f, 0.1f))
		);
		...
		for (auto const &particle : particles) {
			if (particle.isBarrier) {
				auto M = translate(mat4f{1.f}, particle.position);
				addInstance(barrierRenderable, M);
				continue;
			}
